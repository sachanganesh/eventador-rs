...
use stitch_channel::net::StitchAgent;
...
fn test_tls() -> Result<TlsClientAgent, anyhow::Error> {
    ...
    client_config
        ...
        .expect("it works");
    match TlsClientAgent::new(IP_ADDR, DOMAIN, client_config.into()) {
        Ok(data) => Ok(data),
        Err(err) => {
            error!("{}", err);
            panic!(err);
